#!/usr/bin/env node
/**
 * fetch-m65-f-8-sider-rullefalset-import.js
 *
 * Imports one matrix-v1 product from wir-machen-druck:
 *   https://www.wir-machen-druck.de/faltblatt-gefalzt-auf-din-lang-vertikaler-wickelfalz-8seitig.html
 *
 * Product:
 *   - Name: M65 F 8 sider rullefalset
 *   - Format: M65 (10.5 x 21.0 cm) – DIN lang, 8-page roll fold
...
const DEFAULT_PRODUCT_NAME = "M65 F 8 sider rullefalset";
const DEFAULT_PRODUCT_SLUG = "m65-f-8-sider-rullefalset";
...
function usage() {
  return [
    "Usage:",
    "  node scripts/fetch-m65-f-8-sider-rullefalset-import.js import [--dry-run] [--tenant <uuid>] [--name <name>] [--slug <slug>] [--from-targeted-json <path>]",
  ].join("\n");
}
...
async function ensureProduct(client, tenantId, productName, productSlug) {
  ...
  const { data: created, error: createError } = await client
    ...
    .insert({
      ...
      slug: productSlug,
      icon_text: "Folder",
      description: "M65 F 8 sider rullefalset - auto-imported from wir-machen-druck.de",
      category: "tryksager",
      pricing_type: "matrix",
      is_published: false,
      preset_key: "custom",
  ...
  mappedRows.forEach((row) => {
    ...
    const payload = {
      ...
      extra_data: {
        ...
        source: "m65_f_8_sider_rullefalset_fetch_import",
