...
test("reference integrations release gate requires both PostgreSQL 17 suites", async () => {
  const [gate, packageJson, runbook] = await Promise.all([
...
test("runbook pins CVR and DAR staging to the authenticated API boundary", async () => {
  const [shopSettings, runbook] = await Promise.all([
    readFile(join(REPOSITORY_DIR, "src/components/admin/ShopSettings.tsx"), "utf8"),
    readFile(join(REPOSITORY_DIR, "docs/REFERENCE_INTEGRATIONS.md"), "utf8"),
  ]);
  const verifyStart = shopSettings.indexOf("const handleVerifyBusiness");
  const verifyHandler = shopSettings.slice(
    verifyStart,
    shopSettings.indexOf("if (isLoading)", verifyStart),
  );

  assert.match(verifyHandler, /operation:\s*["']vies["']/);
  assert.doesNotMatch(verifyHandler, /danish_company|danish_address/);
  assert.match(runbook, /Datafordeler CVR and DAR[\s\S]*API-harness contract test/);
  assert.match(runbook, /"operation":"danish_company"/);
  assert.match(runbook, /"operation":"danish_address"/);
  assert.match(runbook, /no UI read\/display path/);
});
