#!/usr/bin/env node
/**
 * fetch-tueranhaenger-modern-40-import.js
 *
 * Imports matrix pricing from wir-machen-druck:
 *   https://www.wir-machen-druck.de/tueranhaenger-modern-40farbig-einseitiger-druck-mit-beidseitig-vollflaechiger-uvlackierung.html
 *
 * Product:
 *   - Name: Tueranhaenger modern 4/0
 *   - Vertical axis: material
 *   - One hidden format value: Standard
 *   - Quantities: all available up to 20,000
 */
...
const DEFAULT_PRODUCT_NAME = "Tueranhaenger modern 4/0";
const DEFAULT_PRODUCT_SLUG = "tueranhaenger-modern-4-0";
const DEFAULT_FORMAT_LABEL = "Standard";
const DEFAULT_WIDTH_MM = 95;
...
function usage() {
  return [
    "Usage:",
    "  node scripts/fetch-tueranhaenger-modern-40-import.js import [--dry-run] [--tenant <uuid>] [--name <name>] [--slug <slug>] [--url <supplier-url>] [--format-label <format>] [--width-mm <num>] [--height-mm <num>] [--from-targeted-json <path>]",
  ].join("\n");
}
...
async function extractTargetedRows(page, sourceUrl) {
  ...
  try {
    ...
    // cookie banner not always visible
  }
  ...
  if (materialOptions.length === 0) {
    throw new Error("No supplier material options found in #sorten");
  }
  ...
  for (const material of materialOptions) {
    ...
    qtyTexts.forEach((text) => {
      ...
      if (!parsed) return;
      if (parsed.quantity > MAX_QUANTITY) return;
    ...
    const parsedRows = Array.from(perMaterialMap.values()).sort(
      (a, b) => a.quantity - b.quantity
    );
    extractedRows.push(...parsedRows);
    console.log(
      `  ${material.label.substring(0, 74).padEnd(74)} -> found ${parsedRows.length
        .toString()
        .padStart(2, " ")} quantities`
    );
  }

  return {
    extractedRows,
    materialLabels: materialOptions.map((item) => item.label),
  };
}
...
function buildTransformedSourceRows(sourceRows) {
  ...
  sourceRows.forEach((row) => {
    ...
    dedup.set(`${row.sourceMaterialLabel}||${row.quantity}`, {
      ...row,
...
async function ensureProduct(
  ...
  const { data: created, error: createError } = await client
    ...
    .insert({
      ...
      name: productName,
      slug: productSlug,
      icon_text: "Doorhanger",
      description: "Tueranhaenger modern 4/0 - auto-imported from wir-machen-druck.de",
      category: "tryksager",
      pricing_type: "matrix",
      is_published: false,
      preset_key: "custom",
...
function buildPricingStructure({ materialGroup, materialValues, formatGroup, formatValue }) {
  const quantities = Array.from(
    new Set(materialValues.flatMap((value) => value.__quantities || []))
  ).sort((a, b) => a - b);
...
async function importToSupabase({
  ...
  const activeMaterialLabels = Array.from(
    new Set(mappedRows.map((row) => row.materialLabel))
  );
  const quantities = Array.from(new Set(mappedRows.map((row) => row.quantity))).sort(
    (a, b) => a - b
  );
  ...
  mappedRows.forEach((row) => {
    ...
    const payload = {
      ...
      extra_data: {
        ...
        source: "tueranhaenger_modern_40_fetch_import",
...
async function main() {
  ...
  console.log("=== Tueranhaenger Modern 4/0 Import ===");
  ...
  if (args.fromTargetedJson) {
    ...
    try {
      ...
      materialLabels = extractionResult.materialLabels;
    } finally {
      await browser.close();
    }
  }
  ...
  const slugDir = kebabCase(args.productSlug || "tueranhaenger-modern-4-0");
