#!/usr/bin/env node
/**
 * fetch-m65-folder-midterfalset-import.js
 *
 * Imports one matrix-v1 product from wir-machen-druck:
 *   https://www.wir-machen-druck.de/faltblatt-gefalzt-auf-din-lang-105-cm-x-210-cm-vertikaler-mittelfalz-4seitig.html
 *
 * Product:
 *   - Name: M65 folder midterfalset
 *   - Format: M65 (10.5 x 21 cm)
...
const DEFAULT_PRODUCT_NAME = "M65 folder midterfalset";
const DEFAULT_PRODUCT_SLUG = "m65-folder-midterfalset";
const DEFAULT_FORMAT_LABEL = "M65";
const DEFAULT_WIDTH_MM = 105;
...
const OPTIONAL_SOURCE_LABELS = Array.from(
  new Set([
...
const TARGET_SOURCE_LABELS = Array.from(
  new Set([...REQUIRED_SOURCE_LABELS, ...OPTIONAL_SOURCE_LABELS])
);

function usage() {
  return [
    "Usage:",
    "  node scripts/fetch-m65-folder-midterfalset-import.js import [--dry-run] [--tenant <uuid>] [--name <name>] [--slug <slug>] [--url <supplier-url>] [--format-label <format>] [--width-mm <num>] [--height-mm <num>] [--from-targeted-json <path>]",
  ].join("\n");
}
...
async function extractTargetedRows(page, sourceUrl) {
  ...
  const missingRequiredLabels = REQUIRED_SOURCE_LABELS.filter(
    (label) => !materialByLabel.has(label)
  );
  if (missingRequiredLabels.length > 0) {
    throw new Error(
      `Missing required supplier materials: ${missingRequiredLabels.slice(0, 5).join(" | ")}${
        missingRequiredLabels.length > 5 ? ` (+${missingRequiredLabels.length - 5} more)` : ""
      }`
    );
  }

  const missingOptionalLabels = OPTIONAL_SOURCE_LABELS.filter(
    (label) => !materialByLabel.has(label)
  );
  if (missingOptionalLabels.length > 0) {
    console.log(
      `Optional supplier materials missing on page: ${missingOptionalLabels.length} (continuing)`
    );
  }

  const availableSourceLabels = TARGET_SOURCE_LABELS.filter((label) =>
    materialByLabel.has(label)
  );

  const extractedRows = [];

  for (const sourceMaterialLabel of availableSourceLabels) {
...
async function ensureProduct(
  ...
  const { data: created, error: createError } = await client
    ...
    .insert({
      ...
      slug: productSlug,
      icon_text: "Folder",
      description: "M65 folder midterfalset - auto-imported from wir-machen-druck.de",
      category: "tryksager",
      pricing_type: "matrix",
      is_published: false,
      preset_key: "custom",
...
function buildPricingStructure({
  ...
  if (uvLakGroup && uvLakValues.length > 0) {
    layoutRows.push({
      id: "row-uv-lak",
      title: "",
      description: "",
  ...
  if (kacheringGroup && kacheringValues.length > 0) {
    layoutRows.push({
      id: "row-kachering",
      title: "",
      description: "",
  ...
  if (glossCachingGroup && glossCachingValues.length > 0) {
    layoutRows.push({
...
async function importToSupabase({
  ...
  const activeMaterialLabels = MATERIAL_ORDER.filter((label) =>
    mappedRows.some((row) => row.materialLabel === label)
  );
  const activeUvLakLabels = Array.from(
    new Set(mappedRows.map((row) => row.uvLakLabel).filter(Boolean))
  );
  const activeKacheringLabels = Array.from(
    new Set(mappedRows.map((row) => row.kacheringLabel).filter(Boolean))
  );
  const activeGlossCachingLabels = Array.from(
    new Set(mappedRows.map((row) => row.glossCachingLabel).filter(Boolean))
  );
  ...
  if (dryRun) {
    return {
      ...
      uniqueUvLakValues: activeUvLakLabels.length,
      uniqueKacheringValues: activeKacheringLabels.length,
      uniqueGlossCachingValues: activeGlossCachingLabels.length,
    };
  }
  ...
  const uvLakGroup =
    activeUvLakLabels.length > 0
      ? await ensureGroup(client, context, {
          name: "UV-lak",
          kind: "finish",
          sortOrder: 3,
        })
      : null;
  const kacheringGroup =
    activeKacheringLabels.length > 0
      ? await ensureGroup(client, context, {
          name: "Kachering",
          kind: "finish",
          sortOrder: 4,
      ...
      : null;
  const glossCachingGroup =
    activeGlossCachingLabels.length > 0
      ? await ensureGroup(client, context, {
          name: "Gloss Caching",
          kind: "finish",
          sortOrder: 5,
  ...
  formatMap.set(
    formatLabel,
    await ensureValue(client, context, formatGroup, formatLabel, {
      widthMm,
      heightMm,
    })
  );
  ...
  if (uvLakGroup) {
    for (const uvLakLabel of activeUvLakLabels) {
      uvLakMap.set(
        uvLakLabel,
  ...
  if (kacheringGroup) {
    for (const kacheringLabel of activeKacheringLabels) {
      kacheringMap.set(
        kacheringLabel,
  ...
  if (glossCachingGroup) {
    for (const glossCachingLabel of activeGlossCachingLabels) {
      glossCachingMap.set(
        glossCachingLabel,
  ...
  const pricingStructure = buildPricingStructure({
    ...
    formatGroup,
    formatValues: [formatMap.get(formatLabel)].filter(Boolean),
    surfaceGroup,
    surfaceValues: SURFACE_VALUES.map((name) => surfaceMap.get(name)).filter(Boolean),
    uvLakGroup,
    uvLakValues: activeUvLakLabels.map((name) => uvLakMap.get(name)).filter(Boolean),
    kacheringGroup,
    kacheringValues: activeKacheringLabels.map((name) => kacheringMap.get(name)).filter(Boolean),
    glossCachingGroup,
    glossCachingValues: activeGlossCachingLabels
      .map((name) => glossCachingMap.get(name))
      .filter(Boolean),
  });
  ...
  mappedRows.forEach((row) => {
    ...
    const payload = {
      ...
      extra_data: {
        ...
        source: "m65_folder_midterfalset_fetch_import",
...
async function runImport(args) {
  ...
  if (args.fromTargetedJson) {
    ...
    availableSourceLabels = Array.from(
      new Set(extractedRows.map((row) => row.sourceMaterialLabel).filter(Boolean))
    ...
    try {
      ...
      availableSourceLabels = extractionResult.availableSourceLabels;
      missingOptionalLabels = extractionResult.missingOptionalLabels;
    } finally {
      await browser.close();
    }
  }
    ...
    JSON.stringify(
      {
        ...
        source_labels: {
          required: REQUIRED_SOURCE_LABELS,
          optional: OPTIONAL_SOURCE_LABELS,
          attempted: TARGET_SOURCE_LABELS,
          available: availableSourceLabels,
          missing_optional: missingOptionalLabels,
        },
