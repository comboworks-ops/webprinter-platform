...
test("snapshot draft writes require a server-only Supabase client boundary", () => {
  const importer = fs.readFileSync(
    new URL("../../fetch2-wmd-roll-labels.mjs", import.meta.url),
    "utf8",
  );
  const start = importer.indexOf("function createSnapshotDraftServiceClient");
  const end = importer.indexOf("function toFiniteNumber", start);
  const boundary = importer.slice(start, end);

  assert.ok(start >= 0 && end > start);
  assert.match(boundary, /process\.env\.SUPABASE_URL/);
  assert.match(boundary, /process\.env\.SUPABASE_SERVICE_ROLE_KEY/);
  assert.match(boundary, /SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY/);
  assert.doesNotMatch(boundary, /VITE_|PUBLISHABLE|ANON/);
  assert.match(
    importer,
    /const client = createSnapshotDraftServiceClient\(\);[\s\S]*?apply_wmd_roll_label_snapshot_draft_import/,
  );
});
