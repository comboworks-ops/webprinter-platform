#!/usr/bin/env node
/**
 * fetch-silketryk-tshirt2-import.mjs
 *
 * Creates/updates one combined t-shirt product with two print scopes:
 ...
 * - Buttons: Trykområde (front only / front+back)
 * - Buttons: Silketryk (1/0 .. 4/0)
 * - Dropdown: Frontposition
 * - Dropdown: Bagposition (includes "Ingen bagtryk" for front-only scope)
 */
...
const FRONT_BACK_URL =
  "https://www.wir-machen-druck.de/tshirt-herren-budget-weiss-fruit-of-the-loom-mit-zwei-druckpositionen.html";

const DEFAULT_TENANT_ID = "00000000-0000-0000-0000-000000000000";
const DEFAULT_PRODUCT_NAME = "Silk T-shirt 2";
const DEFAULT_PRODUCT_SLUG = "silk-t-shirt-2";
const DEFAULT_FORMAT_LABEL = "Standard";
const DEFAULT_WIDTH_MM = 300;
const DEFAULT_HEIGHT_MM = 400;

const PRINT_SCOPE_FRONT_ONLY_LABEL = "Print on front of T-shirt";
const PRINT_SCOPE_FRONT_BACK_LABEL = "Print on back and front of T-shirt";
const NO_BACK_PRINT_LABEL = "Ingen bagtryk";
...
function usage() {
  return [
    "Usage:",
    "  node scripts/fetch-silketryk-tshirt2-import.mjs import [--dry-run] [--tenant <uuid>] [--name <name>] [--slug <slug>] [--front-only-url <url>] [--front-back-url <url>] [--format-label <format>] [--width-mm <num>] [--height-mm <num>]",
  ].join("\n");
}
...
async function detectPositionFields(page) {
  ...
  let candidates = fields.filter((f) => /^zusatzfeld\[\d+\]$/i.test(f.name));
  if (candidates.length === 0) {
    candidates = fields.filter((f) =>
      f.options.some((opt) => /bedruckt|druck/i.test(opt.label || ""))
    );
  }
...
async function extractRowsForScope(page, {
  ...
  const positionFields = await detectPositionFields(page);
  ...
  for (const mode of resolvedModes) {
    ...
    for (const frontPosition of frontOptions) {
      ...
      for (const backPosition of backOptions) {
        ...
        console.log(
          `  ${printScopeLabel.substring(0, 30).padEnd(30)} | ${mode.displayLabel.substring(0, 22).padEnd(22)} | ${frontPosition.label.substring(0, 30).padEnd(30)} | ${String((requireBackPosition ? backPosition.label : NO_BACK_PRINT_LABEL)).substring(0, 24).padEnd(24)} -> ${String(rows.length).padStart(2, " ")} / ${TARGET_QUANTITIES.length}`
        );
      }
    }
  }
...
function buildMappedRows(transformedRows, { formatLabel }) {
  return transformedRows.map((row) => ({
    rowType: "silketryk_tshirt2",
    formatLabel,
...
async function ensureProduct(client, tenantId, productName, productSlug, { formatLabel, widthMm, heightMm }) {
  ...
  const { data: created, error: createError } = await client
    ...
    .insert({
      ...
      slug: productSlug,
      icon_text: "T-shirt",
      description: "Silketryk t-shirt (front + front/back) - auto-imported from wir-machen-druck.de",
...
async function importToSupabase({
  ...
  mappedRows.forEach((row) => {
    ...
    if (!printScopeValue || !printModeValue || !frontPositionValue || !backPositionValue) return;
    ...
    const payload = {
      ...
      extra_data: {
        ...
        source: "silketryk_tshirt2_fetch_import",
        sourceUrl: row.sourceUrl,
        sourceModeLabel: row.sourceModeLabel,
        sourceModeValue: row.sourceModeValue,
        sourceFrontPositionLabel: row.sourceFrontPositionLabel,
...
async function main() {
  ...
  console.log("=== Silk T-shirt 2 Import ===");
  ...
  const slugDir = kebabCase(args.productSlug || "silk-t-shirt-2");
