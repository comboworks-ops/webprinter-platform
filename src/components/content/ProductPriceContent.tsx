...
interface ProductPriceContentProps {
    slug?: string;
}
...
export const ProductPriceContent = ({ slug: propSlug }: ProductPriceContentProps) => {
    ...
    // Use static product if available, otherwise fallback to database product
    ...
    const currentDimensions = useMemo(() => {
        if (selectedFormatId) {
            ...
            if (meta?.width_mm && meta?.height_mm) {
                return {
                    ...
                    bleed: typeof meta.bleed_mm === "number" ? meta.bleed_mm : 3
                };
            }
        }
        ...
        if (variantDims) return { ...variantDims, bleed: 3 };
        return { width: 210, height: 297, bleed: 3 };
    }, [selectedFormatId, selectedFormat, selectedVariantName, valueMetaById, valueNameById]);
    ...
    const designSafeAreaMm = useMemo(() => {
        ...
        return undefined;
    }, [selectedFormatId, valueMetaById]);
    ...
    useEffect(() => {
        async function fetchDbProduct() {
            ...
            const resolvedTenantId = previewTenantId || shopSettings.data?.id;
            if (!previewTenantId && shopSettings.isLoading) return;
            setLoading(true);
            ...
            if (data) {
                ...
                setDbProduct(data);
                setOrderDeliveryConfig(data.banner_config?.order_delivery || null);
                // Check for matrix_layout_v1 (handle JSON strings or missing mode)
                const rawStructure = data.pricing_structure;
    ...
    useEffect(() => {
        ...
        const loadMatrixData = async () => {
            ...
            if (qtyParam && variantParam) {
                ...
                if (row && newMatrixData.columns.includes(qty)) {
                    const price = Math.round(Number(newMatrixData.cells[row]?.[qty]) || 0);
                ...
                if (newMatrixData.rows.length > 0) {
                    ...
                    if (defaultQty > 0) {
                        const price = Math.round(Number(newMatrixData.cells[defaultRow]?.[defaultQty]) || 0);
        ...
        loadMatrixData();
    }, [product, selectedFormat, selectedExtraOption, searchParams, dbProductId, selectedVariantName, isGenericPricing, pricingStructure, valueNameById, isStorformat]);
    ...
    const handleCellClick = async (_row: string, column: number, basePrice: number, displayPrice: number) => {
        ...
        setProductPrice(Math.round(Number(basePrice) || 0));
