...
export function ContentBlocksSection({
    ...
    const isFocusedMode = Boolean(focusedBlockId && focusTargetId);
    const visibleBlocks = focusedBlockId
        ? contentBlocks.filter((block) => block.id === focusedBlockId)
        : contentBlocks;
    ...
    useEffect(() => {
        if (focusedBlockId) {
            const element = document.getElementById(`site-design-focus-content-${focusedBlockId}`);
            if (element) {
    ...
    return (
        <div className="space-y-4">
            {visibleBlocks.map((block) => {
                const originalIndex = contentBlocks.findIndex((entry) => entry.id === block.id);
                const showWholeBlock = !isFocusedMode || focusTargetId === `site-design-focus-content-${block.id}`;
                const showHeading = showWholeBlock || focusTargetId === `site-design-focus-content-${block.id}-heading`;
                const showText = showWholeBlock || focusTargetId === `site-design-focus-content-${block.id}-text`;
                const showImage = showWholeBlock || focusTargetId === `site-design-focus-content-${block.id}-image`;

                return (
                <Card
                    key={block.id}
                    id={`site-design-focus-content-${block.id}`}
                    className={cn(
                        "border-dashed transition-all duration-300",
                        focusedBlockId === block.id && "border-primary shadow-md",
                        focusTargetId === `site-design-focus-content-${block.id}` && "ring-2 ring-primary/50 ring-offset-2"
                    )}
                >
                    <CardHeader className="py-3">
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-sm font-medium">
                                Blok {(originalIndex >= 0 ? originalIndex : 0) + 1}
                            </CardTitle>
                            <div className="flex items-center gap-1">
                                ...
                                <Button
                                    ...
                                    disabled={originalIndex === contentBlocks.length - 1}
                                >
                    ...
                    <CardContent className={cn("space-y-4", !block.enabled && "opacity-50")}>
                        {showHeading && (
                            <div id={`site-design-focus-content-${block.id}-heading`} className="space-y-3">
                        ...
                        {showText && (
                            <div id={`site-design-focus-content-${block.id}-text`} className="space-y-3">
                                <Label>Tekst</Label>
                        ...
                        {showImage && (
                            <div id={`site-design-focus-content-${block.id}-image`} className="space-y-3">
                                <Label>Billede</Label>
                                ...
                                <div className="flex items-center gap-4 rounded-md border p-2">
                                    {block.imageUrl ? (
                                        ...
                                        <label className="flex h-16 w-24 flex-shrink-0 cursor-pointer flex-col items-center justify-center rounded border-2 border-dashed hover:bg-muted/50">
                                            {uploading === block.id ? (
                                    ...
                                    <div className="flex-1 space-y-2">
                                        <div className="flex items-center justify-between gap-3">
                                            <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Placering</Label>
                                            <Select
                                        ...
                                        <div className="flex items-center justify-between gap-3">
                                            <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Justering</Label>
