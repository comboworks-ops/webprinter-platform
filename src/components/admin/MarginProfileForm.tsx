...
export function MarginProfileForm({ open, onOpenChange, profile: initialProfile, tenantId, onSuccess }: MarginProfileFormProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="machine-pricing-dialog left-4 right-4 top-4 z-50 h-[calc(100vh-2rem)] w-auto max-w-none translate-x-0 translate-y-0 overflow-hidden rounded-md border-none bg-slate-50 p-0 shadow-xl data-[state=closed]:slide-out-to-top-[4%] data-[state=open]:slide-in-from-top-[4%] sm:left-10 sm:right-10 sm:top-8 sm:h-[calc(100vh-4rem)]">
        <form onSubmit={handleSubmit} className="flex h-full min-h-0 flex-col">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  {isDirty ? (
                    <Badge className="rounded-sm bg-amber-100 px-3 py-1 text-amber-700 hover:bg-amber-100">
                      Ugemte ændringer
                    </Badge>
                  ) : null}
                </div>
              ...
              <div className="grid gap-2 text-right text-sm text-slate-500 sm:grid-cols-3">
                ...
                <div className="rounded-md border border-slate-200 bg-slate-50 px-4 py-3">
                  ...
                  <div className="mt-1 font-semibold text-slate-700">{minRate}% - {maxRate}%</div>
                </div>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
            <div className="grid min-h-full gap-6 px-4 py-5 lg:grid-cols-[minmax(0,1fr)_320px] lg:px-6">
              <div className="space-y-6">
                <section className="rounded-md border border-slate-200 bg-white p-6 shadow-none">
                  ...
                  <div className="grid gap-5 md:grid-cols-2">
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <Input
                        ...
                        className="h-12 rounded-md border-slate-200 bg-slate-50 shadow-none"
                        required
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Select value={formData.mode} onValueChange={(value) => setFormData({ ...formData, mode: value })}>
                        <SelectTrigger className="h-12 rounded-md border-slate-200 bg-slate-50 shadow-none">
                          <SelectValue />
                        </SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <NumberInput
                          ...
                          onValueChange={(value) => setFormData({ ...formData, rounding_step: value })}
                          className="h-12 rounded-md border-slate-200 bg-slate-50 pr-16 shadow-none"
                        />
                ...
                <section className="rounded-md border border-slate-200 bg-white p-6 shadow-none">
                  <div className="mb-5 flex items-center justify-between gap-4">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={addTier} className="gap-2 rounded-md">
                  ...
                  <div className="space-y-4">
                    {tiers.map((tier, index) => (
                      <div key={index} className="rounded-md border border-slate-200 bg-white/80 p-4 shadow-none">
                        ...
                        <div className="grid gap-4 md:grid-cols-[1fr_1fr_1fr]">
                          <div className="space-y-2">
                            ...
                            <NumberInput
                              value={tier.qty_from}
                              onValueChange={(value) => updateTier(index, "qty_from", value)}
                              className="h-11 rounded-md border-slate-200 bg-slate-50 shadow-none"
                            />
                          </div>
                          <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="relative">
                              <NumberInput
                                value={tier.value}
                                onValueChange={(value) => updateTier(index, "value", value)}
                                className="h-11 rounded-md border-slate-200 bg-slate-50 pr-12 shadow-none"
                              />
              ...
              <aside className="space-y-5 lg:sticky lg:top-5">
                <div className="rounded-md border border-slate-200 bg-white p-5 shadow-none">
                  <div className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Profil-preview</div>
                  <div className="mt-4 space-y-4 text-sm">
                    <div className="flex items-start gap-3 rounded-md border border-slate-100 bg-slate-50 p-4">
                    ...
                    <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
                      <div className="text-[11px] uppercase tracking-[0.18em] text-slate-700">Spænd i profil</div>
          ...
          <DialogFooter className="border-t border-slate-200 bg-white px-6 py-4 sm:justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button type="submit" className="rounded-md bg-primary px-5 hover:bg-primary/90">
                Gem profil
              </Button>
            </div>
          </DialogFooter>
