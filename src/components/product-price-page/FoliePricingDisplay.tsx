...
const materials = [
  { id: "Mat", label: "Mat" },
  { id: "Glans", label: "Glans" },
];

export function FoliePricingDisplay({
  onPriceChange,
  initialWidth,
  initialHeight,
  initialMaterial
}: FoliePricingDisplayProps) {
  ...
  const [foilPrices, setFoilPrices] = useState<FoilPrice[]>([]);

  useEffect(() => {
    fetchFoilPrices();
  }, []);

  const fetchFoilPrices = async () => {
    try {
  ...
  useEffect(() => {
    ...
    // Validate dimensions (in cm, max 500x500)
  ...
  }, [width, height, selectedMaterial, onPriceChange, foilPrices]);
