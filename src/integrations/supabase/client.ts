...
import { IS_TEST_DEPLOYMENT, testDeploymentRequestBlock } from '@/lib/testDeployment';
import {
  migrateLocalKeysToTransient,
  removeLocalStorageKeysByPrefix,
} from '@/lib/storage/transientStorage';
...
const AUTH_FALLBACK_STORAGE_PREFIX = 'wp_auth_fallback:';
const PRUNABLE_LOCAL_STORAGE_KEYS = [
  'branding-draft-unsaved',
  'master_branding_draft',
  'designer_proofing_settings',
];
...
const clearLocalSupabaseSession = () => {
  if (typeof window !== 'undefined') {
    ...
    if (key) {
      ...
      window.sessionStorage.removeItem(`${AUTH_FALLBACK_STORAGE_PREFIX}${key}`);
      window.sessionStorage.removeItem(`${AUTH_FALLBACK_STORAGE_PREFIX}${key}-code-verifier`);
    }
  }
  if (supabaseClient) {
...
const createQuotaSafeAuthStorage = (): Storage | undefined => {
  if (typeof window === 'undefined') return undefined;

  const readFallback = (key: string) => window.sessionStorage.getItem(`${AUTH_FALLBACK_STORAGE_PREFIX}${key}`);
  const writeFallback = (key: string, value: string) =>
    window.sessionStorage.setItem(`${AUTH_FALLBACK_STORAGE_PREFIX}${key}`, value);
  const removeFallback = (key: string) =>
    window.sessionStorage.removeItem(`${AUTH_FALLBACK_STORAGE_PREFIX}${key}`);

  const pruneNonEssentialLocalStorage = () => {
    for (const storageKey of PRUNABLE_LOCAL_STORAGE_KEYS) {
      try {
        window.localStorage.removeItem(storageKey);
      } catch {
        // Ignore removal failures and continue.
      }
    }
  };
  ...
  return {
    ...
    clear() {
      ...
      for (let index = 0; index < window.sessionStorage.length; index += 1) {
        ...
        if (key?.startsWith(AUTH_FALLBACK_STORAGE_PREFIX)) {
    ...
    setItem(key: string, value: string) {
      ...
      pruneNonEssentialLocalStorage();

      try {
        window.localStorage.setItem(key, value);
        removeFallback(key);
        console.warn('[Supabase Auth] Cleared non-essential local drafts to persist the session token.');
        return;
      } catch (error) {
        if (!(error instanceof DOMException) || error.name !== 'QuotaExceededError') {
          throw error;
      ...
      console.warn('[Supabase Auth] localStorage is full. Auth session is temporarily persisted in sessionStorage.');
    },
  };
};
...
const supabaseFetch: typeof fetch = async (input, init) => {
  const testBlock = testDeploymentRequestBlock(input, IS_TEST_DEPLOYMENT);
...
if (typeof window !== 'undefined') {
  ...
  removeLocalStorageKeysByPrefix([
    'storefront-catalog-cache-v',
    'product-price-detail-cache-v1:',
    'matrix-v1:',
  ]);
}
...
supabaseClient = createClient<Database>(
  ...
  SUPABASE_PUBLISHABLE_KEY || 'placeholder-key',
  {
    ...
    auth: {
      ...
      storage: createQuotaSafeAuthStorage(),
      persistSession: true,
