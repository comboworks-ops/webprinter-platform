...
## Staging interface boundary

At this head, Shop Settings initiates only VIES. Datafordeler CVR and DAR
acceptance is an authenticated Edge-function/API-harness contract test, not a
customer-facing lookup or autofill test. A successful check stores
display-only, tenant-scoped evidence; it never replaces company/address fields
or changes onboarding, VAT, pricing, checkout, login, or access state. CVR
evidence may appear later in the generic evidence panel. DAR evidence currently
has no UI read/display path.

Use these exact request shapes:

```json
{"operation":"danish_company","tenantId":"<uuid>","cvr":"12345678"}
```

```json
{"operation":"danish_address","tenantId":"<uuid>","query":{"streetName":"...","houseNumber":"12B","floor":"","door":"","postcode":"1234","city":"...","country":"DK"}}
```

Both requests require a user JWT and exact tenant authorization. A CVR that
does not match the tenant's saved structured identity is rejected with HTTP
409 before provider work. A cross-tenant request is denied before provider or
persistence work. Missing Datafordeler credentials return HTTP 200 with
sanitized `unavailable` evidence; that is a non-blocking fallback, not a
positive staging-contract pass.
...
Supabase Edge runtime variables used by authenticated handlers:

- `SUPABASE_URL`
- `SUPABASE_ANON_KEY`
- `SUPABASE_SERVICE_ROLE_KEY`

A non-dry-run WMD snapshot-draft CLI write reads Supabase credentials only from
the operator/server process:

- `SUPABASE_URL`
- `SUPABASE_SERVICE_ROLE_KEY`

That snapshot-write boundary does not accept `VITE_*`, publishable, or
anonymous keys. Never check the service-role key into the repository.
...
No PostgreSQL 17 run is recorded for this clean branch's exact head. Historical
results from another branch or SHA are **BLOCKED—not a pass** for this branch.
Before readiness review, run the command on the exact proposed head and retain
all three terminal markers:
...
Static migration assertions, unit tests, Edge tests, and the production build
remain separate gates. If the release owner cannot execute
Docker/PostgreSQL 17, the status remains blocked rather than skipped or green.
