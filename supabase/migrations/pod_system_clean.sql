...
DROP POLICY IF EXISTS "pod_supplier_master_only" ON pod_supplier_connections;
CREATE POLICY "pod_supplier_master_only" ON pod_supplier_connections
  FOR ALL USING (is_pod_master_admin());

DROP POLICY IF EXISTS "pod_presets_master_only" ON pod_api_presets;
CREATE POLICY "pod_presets_master_only" ON pod_api_presets
  FOR ALL USING (is_pod_master_admin());

DROP POLICY IF EXISTS "pod_catalog_master_write" ON pod_catalog_products;
CREATE POLICY "pod_catalog_master_write" ON pod_catalog_products
  FOR ALL USING (is_pod_master_admin());

DROP POLICY IF EXISTS "pod_attrs_master_only" ON pod_catalog_attributes;
CREATE POLICY "pod_attrs_master_only" ON pod_catalog_attributes
  FOR ALL USING (is_pod_master_admin());

DROP POLICY IF EXISTS "pod_vals_master_only" ON pod_catalog_attribute_values;
CREATE POLICY "pod_vals_master_only" ON pod_catalog_attribute_values
  FOR ALL USING (is_pod_master_admin());

DROP POLICY IF EXISTS "pod_matrix_master_only" ON pod_catalog_price_matrix;
CREATE POLICY "pod_matrix_master_only" ON pod_catalog_price_matrix
  FOR ALL USING (is_pod_master_admin());

-- ============================================================
-- 6. VIEW FOR TENANT ACCESS TO PUBLISHED CATALOG
-- ============================================================

DROP VIEW IF EXISTS pod_catalog_public;
CREATE VIEW pod_catalog_public AS
SELECT 
  id,
  status,
  public_title,
...
DROP POLICY IF EXISTS "pod_imports_tenant" ON pod_tenant_imports;
...
DROP POLICY IF EXISTS "pod_billing_tenant" ON pod_tenant_billing;
...
DROP POLICY IF EXISTS "pod_jobs_tenant" ON pod_fulfillment_jobs;
...
DROP TRIGGER IF EXISTS pod_supplier_updated ON pod_supplier_connections;
...
DROP TRIGGER IF EXISTS pod_catalog_updated ON pod_catalog_products;
CREATE TRIGGER pod_catalog_updated BEFORE UPDATE ON pod_catalog_products
  FOR EACH ROW EXECUTE FUNCTION update_pod_updated_at();

DROP TRIGGER IF EXISTS pod_matrix_updated ON pod_catalog_price_matrix;
CREATE TRIGGER pod_matrix_updated BEFORE UPDATE ON pod_catalog_price_matrix
  FOR EACH ROW EXECUTE FUNCTION update_pod_updated_at();

DROP TRIGGER IF EXISTS pod_billing_updated ON pod_tenant_billing;
CREATE TRIGGER pod_billing_updated BEFORE UPDATE ON pod_tenant_billing
  FOR EACH ROW EXECUTE FUNCTION update_pod_updated_at();

DROP TRIGGER IF EXISTS pod_jobs_updated ON pod_fulfillment_jobs;
CREATE TRIGGER pod_jobs_updated BEFORE UPDATE ON pod_fulfillment_jobs
  FOR EACH ROW EXECUTE FUNCTION update_pod_updated_at();

-- ============================================================
-- DONE! POD system tables created.
-- ============================================================
