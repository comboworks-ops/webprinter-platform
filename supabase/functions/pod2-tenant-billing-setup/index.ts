  ...
  try {
    ...
    if (existingBilling?.stripe_customer_id) {
      ...
      const customer = await stripe.customers.create({
        metadata: {
          tenant_id: tenantId,
          platform: "webprinter_pod2",
        },
        name: tenant?.name || `Tenant ${tenantId}`,
        description: `Webprinter POD v2 - ${tenant?.domain || tenantId}`,
      });

      stripeCustomerId = customer.id;

      const { error: billingInsertError } = await serviceClient
        .from("pod2_tenant_billing")
        .upsert({
          tenant_id: tenantId,
    ...
    console.error("POD2 Billing Setup error:", error);
